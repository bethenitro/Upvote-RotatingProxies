...
def attempt_proxy_rotation(proxy: Dict[str, Any], custom_logger: logging.Logger = None) -> Optional[Dict[str, Any]]:
    """
    ...
    if not can_rotate_proxy(proxy):
        log.debug(f"Skipping rotation for {proxy['server']}: Too soon since last rotation")
        return None

    try:
        log.info(f"Attempting to rotate IP for proxy: {proxy['server']}")
        ...
        if response_data.get("success") == 1:
            ...
            log.info(f"Successfully rotated IP for {proxy['server']}: {response_data.get('message')}")
            return {
                "server": proxy["server"],
            ...
            log.warning(f"Failed to rotate proxy {proxy['server']}: {error_msg}")
        ...
        log.error(f"Error rotating proxy {proxy['server']}: {str(e)}")
        return None

def rotate_proxy(custom_logger: logging.Logger = None) -> Dict[str, Any]:
    """
    ...
    while attempt < max_attempts:
        log.debug(f"Proxy rotation attempt {attempt + 1}/{max_attempts}")
        ...
        if not rotatable_proxies:
            ...
            log.info(f"All proxies are on cooldown. Waiting {wait_time} seconds...")
        ...
        for proxy in rotatable_proxies:
            rotated_proxy = attempt_proxy_rotation(proxy, log)
            if rotated_proxy:  # Only return if rotation was successful
                log.info(f"Successfully obtained rotated proxy: {proxy['server']}")
            ...
            if proxy["server"] in last_rotation_times:
                ...
                log.info(f"Proxy {proxy['server']} is on cooldown. Waiting {wait_time} seconds...")
                time.sleep(wait_time)
                
        attempt += 1
                
    # If we've exhausted all attempts
    log.error("Failed to rotate any proxy after multiple attempts")
